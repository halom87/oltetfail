...
// sajat
uint8_t initSensorACC(void)
{
	WriteReg(ACC_I2C_ADDRESS,CTRL_REG2_A,0x00);//No high pass filter
	WriteReg(ACC_I2C_ADDRESS,CTRL_REG3_A,0x00);//No interrupt
	WriteReg(ACC_I2C_ADDRESS,CTRL_REG4_A,0b10001000);//Registers not update until read ,LSB at lower address High resolution
	WriteReg(ACC_I2C_ADDRESS,CTRL_REG5_A,0b00000000);//FIFO disable
	WriteReg(ACC_I2C_ADDRESS,CTRL_REG6_A,0x00);
	WriteReg(ACC_I2C_ADDRESS,CTRL_REG1_A,0b01110111);//400Hz 3Axis enable
...
void inline readRegACC(uint8_t reg, uint8_t *data)
{
	ReadReg(ACC_I2C_ADDRESS,reg,data);
}
