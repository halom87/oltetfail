...
#include "HRDWCFG.h"             /* ST Setup and Utilities      */
...
uint8_t recvBuff[BT_BUFFER_SIZE];
static void RxProcess(void)
{
   ...
   Count = UART_GetReceivedBytesCount();
   if(Count)
   {
	   UART_TryReceive(recvBuff, 0, Count, 0);

      /* Call the upper layer back with the data.                       */
      if(_COMDataCallback)
         (*_COMDataCallback)(TRANSPORT_ID, Count, recvBuff, _COMCallbackParameter);
   }
}
...
int BTPSAPI HCITR_COMOpen(HCI_COMMDriverInformation_t *COMMDriverInformation, HCITR_COMDataCallback_t COMDataCallback, unsigned long CallbackParameter)
{
   ...
   // KVADRO
   // COM PORT IS OPENED AND CONFIGURED IN THE DRIVER FOLDER AS UART

   /* First, make sure that valid COMM Driver Information was specified.                 */
   if((COMMDriverInformation) && (COMDataCallback))
   {
...
int BTPSAPI HCITR_COMWrite(unsigned int HCITransportID, unsigned int Length, unsigned char *Buffer)
{
   ...
   if((HCITransportID == TRANSPORT_ID) && (HCITransportOpen) && (Length) && (Buffer))
   {
	  ...
	  UART_StartSend(Buffer, 0, Length);
