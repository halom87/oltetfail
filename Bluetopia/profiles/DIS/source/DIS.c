...
typedef struct _tagDIS_Instance_Data_t
{
   ...
   Byte_t                   System_ID_Buffer[DIS_SYSTEM_ID_DATA_SIZE];
...
static BTPSCONST GATT_Characteristic_Declaration_16_Entry_t DIS_Manufacturer_Name_Declaration =
{
   ...
   DIS_MANUFACTURER_NAME_CHARACTERISTIC_UUID_CONSTANT
};
...
{
   GATT_CHARACTERISTIC_PROPERTIES_READ,
   DIS_MODEL_NUMBER_CHARACTERISTIC_UUID_CONSTANT
};
...
{
   GATT_CHARACTERISTIC_PROPERTIES_READ,
   DIS_SERIAL_NUMBER_CHARACTERISTIC_UUID_CONSTANT
};
...
{
   GATT_CHARACTERISTIC_PROPERTIES_READ,
   DIS_HARDWARE_REVISION_CHARACTERISTIC_UUID_CONSTANT
};
...
{
   GATT_CHARACTERISTIC_PROPERTIES_READ,
   DIS_FIRMWARE_REVISION_CHARACTERISTIC_UUID_CONSTANT
};
...
{
   GATT_CHARACTERISTIC_PROPERTIES_READ,
   DIS_SOFTWARE_REVISION_CHARACTERISTIC_UUID_CONSTANT
};
...
{
   GATT_CHARACTERISTIC_PROPERTIES_READ,
   DIS_SYSTEM_ID_CHARACTERISTIC_UUID_CONSTANT
};
...
{
   GATT_CHARACTERISTIC_PROPERTIES_READ,
   DIS_IEEE_CERTIFICATION_DATA_CHARACTERISTIC_UUID_CONSTANT
};
   ...
   if((BluetoothStackID) && (ServiceID))
   {
      ...
      if(!InstanceRegisteredByStackID(BluetoothStackID))
      {
         ...
         if((ServiceInstance = AcquireServiceInstance(BluetoothStackID, &InstanceID)) != NULL)
         {
            ...
            ret_val = GATT_Register_Service(BluetoothStackID, DIS_SERVICE_FLAGS, DEVICE_INFORMATION_SERVICE_ATTRIBUTE_COUNT, (GATT_Service_Attribute_Entry_t *)Device_Information_Service, &ServiceHandleGroup, GATT_ServerEventCallback, InstanceID);
            if(ret_val > 0)
            {
               ...
               *ServiceID                                     = (unsigned int)ret_val;
               ...
               InstanceData[InstanceID-1].System_ID.Characteristic_Length = DIS_SYSTEM_ID_DATA_SIZE;
...
int BTPSAPI DIS_Query_Software_Revision(unsigned int BluetoothStackID, unsigned int InstanceID, char *SoftwareRevision)
{
   ...
   /* error occurs.                                                     */
int BTPSAPI DIS_Set_System_ID(unsigned int BluetoothStackID, unsigned int InstanceID, DIS_System_ID_t *SystemID)
{
#if BTPS_CONFIGURATION_DIS_SUPPORT_SYSTEM_ID
   ...
   if((BluetoothStackID) && (InstanceID) && (SystemID))
   {
      ...
      {
         /* Set the System ID.                                          */
         BTPS_MemCopy(InstanceData[InstanceID-1].System_ID.Data, SystemID, DIS_SYSTEM_ID_DATA_SIZE);
...
int BTPSAPI DIS_Query_System_ID(unsigned int BluetoothStackID, unsigned int InstanceID, DIS_System_ID_t *SystemID)
{
#if BTPS_CONFIGURATION_DIS_SUPPORT_SYSTEM_ID
   ...
   if((BluetoothStackID) && (InstanceID) && (SystemID))
   {
      ...
      {  
         ...
         BTPS_MemCopy(SystemID, InstanceData[InstanceID-1].System_ID.Data, DIS_SYSTEM_ID_DATA_SIZE);
