...
static BTPSCONST GATT_Characteristic_Declaration_16_Entry_t GAP_Device_Name_Declaration =
{
   ...
   GAP_DEVICE_NAME_CHARACTERISTIC_UUID_CONSTANT
};
...
{
   GATT_CHARACTERISTIC_PROPERTIES_READ,
   GAP_DEVICE_APPEARANCE_CHARACTERISTIC_UUID_CONSTANT
};
...
static GATT_Characteristic_Value_16_Entry_t  GAP_Device_Appearence_Value = 
{
   ...
   /* The Peripheral Preferred Connection Parameters Characteristic Declaration.                 */
static BTPSCONST GATT_Characteristic_Declaration_16_Entry_t GAP_Preferred_Connection_Parameters_Declaration =
{
   GATT_CHARACTERISTIC_PROPERTIES_READ,
   GAP_PERIPHERAL_PREFERRED_CONNECTION_PARAMETERS_CHARACTERISTIC_UUID_CONSTANT
};

   /* The Peripheral Preferred Connection Parameters Characteristic Value.                       */
   ...
   if((BluetoothStackID) && (ServiceID))
   {
      ...
      if(!InstanceRegisteredByStackID(BluetoothStackID))
      {
         ...
         if((ServiceInstance = AcquireServiceInstance(BluetoothStackID, &InstanceID)) != NULL)
         {
            ...
            ret_val = GATT_Register_Service(BluetoothStackID, GAP_SERVICE_FLAGS, GENERIC_ACCESS_PROFILE_SERVICE_ATTRIBUTE_COUNT, (GATT_Service_Attribute_Entry_t *)Generic_Access_Profile_Service, &ServiceHandleGroup, GATT_ServerEventCallback, InstanceID);
            if(ret_val > 0)
            {
   ...
   if((BluetoothStackID) && (InstanceID))
   {
      ...
      if((ServiceInstance = AcquireServiceInstance(BluetoothStackID, &InstanceID)) != NULL)
      {
         ...
         if(ServiceInstance->ServiceID)
         {
            ...
            BTPS_MemInitialize(ServiceInstance, 0, GAP_INSTANCE_DATA_SIZE);
