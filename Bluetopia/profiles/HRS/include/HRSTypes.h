...
#define HRS_ASSIGN_HRS_SERVICE_UUID_16(_x)               ASSIGN_UUID_16(*((UUID_16_t *)(_x)), 0x0D, 0x18)
...
#define HRS_COMPARE_HRS_SERVICE_UUID_TO_UUID_16(_x)      COMPARE_UUID_16_TO_CONSTANT((_x), 0x0D, 0x18)

   /* The following defines the Heart Rate Service UUID that is used    */
   /* when building the HRS Service Table.                              */
#define HRS_SERVICE_UUID_CONSTANT                        { 0x0D, 0x18 }

   /* The following MACRO is a utility MACRO that assigns the HRS       */
   /* Heart Rate Measurement Characteristic 16 bit UUID to the         */
   /* specified UUID_16_t variable.  This MACRO accepts one parameter   */
   /* which is the UUID_16_t variable that is to receive the HRS        */
   /* Heart Rate Measurement UUID Constant value.                      */
   /* * NOTE * The UUID will be assigned into the UUID_16_t variable in */
   /*          Little-Endian format.                                    */
#define HRS_ASSIGN_HEART_RATE_MEASUREMENT_UUID_16(_x)    ASSIGN_UUID_16((_x), 0x37, 0x2A)
   ...
   /* UUID.                                                             */
#define HRS_COMPARE_HRS_HEART_RATE_MEASUREMENT_UUID_TO_UUID_16(_x) COMPARE_UUID_16_TO_CONSTANT((_x), 0x37, 0x2A)

   /* The following defines the HRS Heart Rate Measurement              */
   /* Characteristic UUID that is used when building the HRS Service    */
   /* Table.                                                            */
#define HRS_HEART_RATE_MEASUREMENT_CHARACTERISTIC_UUID_CONSTANT    { 0x37, 0x2A }
   ...
   /* * NOTE * The UUID will be assigned into the UUID_16_t variable in */
   /*          Little-Endian format.                                    */
#define HRS_ASSIGN_BODY_SENSOR_LOCATION_UUID_16(_x) ASSIGN_UUID_16((_x), 0x38, 0x2A)
   ...
   /* UUID.                                                             */
#define HRS_COMPARE_HRS_BODY_SENSOR_LOCATION_UUID_TO_UUID_16(_x) COMPARE_UUID_16_TO_CONSTANT((_x), 0x38, 0x2A)

   /* The following defines the HRS Body Sensor Location Characteristic */
   /* UUID that is used when building the HRS Service Table.            */
#define HRS_BODY_SENSOR_LOCATION_CHARACTERISTIC_UUID_CONSTANT    { 0x38, 0x2A }
   ...
   /* * NOTE * The UUID will be assigned into the UUID_16_t variable in */
   /*          Little-Endian format.                                    */
#define HRS_ASSIGN_HEART_RATE_CONTROL_POINT_UUID_16(_x)  ASSIGN_UUID_16((_x), 0x39, 0x2A)
   ...
   /* UUID.                                                             */
#define HRS_COMPARE_HRS_HEART_RATE_CONTROL_POINT_UUID_TO_UUID_16(_x) COMPARE_UUID_16_TO_CONSTANT((_x), 0x39, 0x2A)

   /* The following defines the HRS Heart Rate Control Point            */
   /* Characteristic UUID that is used when building the HRS Service    */
   /* Table.                                                            */
#define HRS_HEART_RATE_CONTROL_POINT_CHARACTERISTIC_UUID_CONSTANT    { 0x39, 0x2A }
...
#define HRS_HEART_RATE_MEASUREMENT_HEADER_DATA_SIZE      (sizeof(HRS_Heart_Rate_Measurement_Header_t))
   ...
   /* The following structure represents a Generic Heart Rate Measurement*/
   /* value with the variable data that it contains.                    */
...
#define HRS_CLIENT_CHARACTERISTIC_CONFIGURATION_VALUE_LENGTH (WORD_SIZE)
