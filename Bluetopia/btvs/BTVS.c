...
int BTPSAPI VS_Set_Max_Output_Power(unsigned int BluetoothStackID, Byte_t MaxPower)
{
   ...
   if((BluetoothStackID) && (MaxPower >= 0) && (MaxPower <= 12))
   {
      ...
      if(!ret_val)
      {
         /* Format the HCI_VS_DRPb_Set_Class2_Single_Power Command.     */
         ...
         /* Send the HCI_VS_DRPb_Set_Class2_Single_Power Vendor         */
         /* Specific Command.                                           */
         Length  = sizeof(ReturnBuffer);
         ...
         {
            /* Send the HCI_VS_DRPb_Set_RF_Calibration_Info Vendor      */
            /* Specific Command.                                        */
            Length  = sizeof(ReturnBuffer);
            ...
            if((!ret_val) && (!Status) && (Length >= 1) && (!ReturnBuffer[0]))
            {
               /* Send the first of two HCI_VS_DRPb_Enable_Calibration  */
               ...
               if((!ret_val) && (!Status) && (Length >= 1) && (!ReturnBuffer[0]))
               {
                  /* Send the second of two HCI_VS_DRPb_Enable_         */
                  /* Calibration Vendor Specific Commands.              */
